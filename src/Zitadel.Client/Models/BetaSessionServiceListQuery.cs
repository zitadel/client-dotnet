// Zitadel SDK
// The Zitadel SDK is a convenience wrapper around the Zitadel APIs to assist you in integrating with your Zitadel environment. This SDK enables you to handle resources, settings, and configurations within the Zitadel platform.
//
// The version of the OpenAPI document: 1.0.0
//
// Generated by: https://openapi-generator.tech

using System.Text.Json.Serialization;

namespace Zitadel.Client.Models;

public class BetaSessionServiceListQuery : IEquatable<BetaSessionServiceListQuery>
{
    /// <example>null</example>
    [JsonPropertyName("offset")]
    public Object? Offset { get; set; }

    /// <example>null</example>
    [JsonPropertyName("limit")]
    public int? Limit { get; set; }

    /// <example>null</example>
    [JsonPropertyName("asc")]
    public bool? Asc { get; set; }

    /// <summary>Value-equality based on all declared fields.</summary>
    public bool Equals(BetaSessionServiceListQuery? other)
    {
        return other is not null
            && (
                ReferenceEquals(this, other)
                || EqualityComparer<Object?>.Default.Equals(this.Offset, other.Offset)
                    && EqualityComparer<int?>.Default.Equals(this.Limit, other.Limit)
                    && EqualityComparer<bool?>.Default.Equals(this.Asc, other.Asc)
            );
    }

    public override bool Equals(object? obj)
    {
        return Equals(obj as BetaSessionServiceListQuery);
    }

    public override int GetHashCode()
    {
        HashCode hash = default;
        hash.Add(this.Offset);
        hash.Add(this.Limit);
        hash.Add(this.Asc);
        return hash.ToHashCode();
    }
}
