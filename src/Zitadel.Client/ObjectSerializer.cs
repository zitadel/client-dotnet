...
internal class ObjectSerializer
{
    ...
    public string Serialize(object? value)
    {
        if (value != null)
        {
            ...
            if (!isAnyOfWrapper)
            {
                System.Reflection.PropertyInfo? actualProp = valueType.GetProperty(
                    "ActualInstance"
                );
                if (actualProp != null)
                {
        ...
        )
        {
            ...
            if (reader.TokenType != JsonTokenType.String)
            {
                throw new JsonException(
                    $"Expected a JSON string for a date-time but got {reader.TokenType}."
                );
            }
    ...
    public static bool StructuralEquals(object? left, object? right)
    {
        ...
        if (
            left is IEnumerable leftSeq
            && right is IEnumerable rightSeq
            && left is not string
            && right is not string
        )
        {
        ...
        if (value is IDictionary dict)
        {
            ...
            foreach (DictionaryEntry entry in dict)
            {
                dictHash ^= HashCode.Combine(
                    StructuralHashCode(entry.Key),
                    StructuralHashCode(entry.Value)
                );
            }
