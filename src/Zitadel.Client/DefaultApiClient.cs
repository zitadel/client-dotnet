...
public sealed class DefaultApiClient : IApiClient, IDisposable
{
    ...
    )
    {
        ...
        {
            throw new ArgumentException(
                $"Invalid Content-Type header value: {contentType}",
                nameof(headers)
            );
        }
        ...
        if (body != null)
        {
            request.Content = body switch
            {
                ...
                Dictionary<string, object> formParts => BuildMultipartContent(
                    formParts,
                    _serializer
                ),
            ...
            if (contentType != null && body is not Dictionary<string, object> && body is not string)
            {
    ...
    private static Encoding GetEncodingFromContentType(string? contentType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(media.CharSet))
            {
                ...
                if (
                    string.Equals(charset, "utf-16", StringComparison.OrdinalIgnoreCase)
