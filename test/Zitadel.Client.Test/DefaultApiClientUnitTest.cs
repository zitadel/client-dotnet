...
public class DefaultApiClientUnitTest
{
    ...
    public async Task LyingGzipContentEncodingSurfacesApiException()
    {
        ...
        Assert.False(ex is Zitadel.Client.Errors.NetworkException, "a corrupt body is not a network failure");
    }
    ...
    public async Task UseAfterCloseThrowsInvalidOperationException()
    {
        ...
        Assert.False(typeof(ZitadelException).IsInstanceOfType(ex), "use-after-close must not be an SDK error");
    }
    ...
    public void NonexistentCaCertPathThrowsArgumentExceptionAtConstruction()
    {
        ...
        Assert.False(typeof(ZitadelException).IsInstanceOfType(ex), "a configuration mistake must not be an SDK error");
        Assert.NotNull(ex.InnerException);
    }
    ...
    public async Task RequestTimeoutRaisesNetworkTimeoutException()
    {
        ...
        Assert.IsAssignableFrom<Zitadel.Client.Errors.NetworkException>(ex);
        Assert.Equal(0, ex.StatusCode);
    ...
    public async Task CallerCancellationPropagatesUnwrapped()
    {
        ...
        Assert.False(typeof(ZitadelException).IsInstanceOfType(ex), "cancellation must not be rewrapped as an SDK error");
    }
