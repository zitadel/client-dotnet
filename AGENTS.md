...
- Generator: `csharp-plus`, run from the `openapi-generator-plus:enhanced` image.
...
Hand-written here: the bespoke authenticators under `src/Zitadel.Client/Auth/`, their tests, the
...
devbox run -- make test       # dotnet test
...
the generator supports. The types live in `Zitadel.Client.Errors`:
...
the trace-context tests). This repo keeps its own `Directory.Build.props`, so the generator cannot add
